# -*- coding: utf-8 -*-
"""
Created on Sat Aug  1 11:52:56 2020
...
#    data= raw_data.copy()
    ...
    def freq(dataset,reference_date,f,g,l):
        a = dataset[dataset['InvoiceDate']>=reference_date].groupby(['customer_id','item_id'],as_index=False).agg({'transaction_id':'count','item_amt':['sum', 'min', 'max'],'item_qty':['sum', 'min', 'max']})#.rename(columns={'transaction_id':f,'item_amt':g,'item_qty':l})
        a.columns = ['customer_id', 'item_id', f, g+'sum', g+'min', g+'max', l+'sum', l+'min', l+'max']
        return a
    ...
    def cust_freq(dataset,reference_date,f,g,l):
        a = dataset[dataset['InvoiceDate']>=reference_date].groupby(['customer_id'],as_index=False).agg({'transaction_id':'count','item_amt':['sum', 'min', 'max'],'item_qty':['sum', 'min', 'max']})#.rename(columns={'transaction_id':f,'item_amt':g,'item_qty':l})
        a.columns = ['customer_id', f, g+'_sum', g+'_min', g+'_max', l+'_sum', l+'_min', l+'_max']
        return a

    c1=cust_freq(data,rd1,'CF1','CM1','CQ1')
    c7=cust_freq(data,rd7,'CF7','CM7','CQ7')
    c30=cust_freq(data,rd30,'CF30','CM30','CQ30')
    c60=cust_freq(data,rd60,'CF60','CM60','CQ60')
    c90=cust_freq(data,rd90,'CF90','CM90','CQ90')
    c360=cust_freq(data,rd360,'CF360','CM360','CQ360')
    c720=cust_freq(data,rd720,'CF720','CM720','CQ720')
    ...
    data_processed = pd.merge(pd.merge(pd.merge(pd.merge(d,c_T_F,how='left',on='customer_id'),c_T_L,how='left',on=['customer_id']),c,how='left',on=['customer_id']),c_tx_day_diff,how='left',on=['customer_id'])
...
for n, col in enumerate(dependent_variables):
    ...
    cols_to_include = [x for x in data.columns if x not in dependent_variables]
#    cols_to_include = cols_to_include + ['T_L', 'T_F', 'CF1', 'CM1', 'CQ1',	'CF7', 'CM7', 'CQ7', 'CF30', 'CM30', 'CQ30', 'CF60', 'CM60', 'CQ60', 'CF90', 'CM90', 'CQ90', 'CF360', 'CM360', 'CQ360',	'CF720', 'CM720', 'CQ720', 'C_DayDiffMean',	'C_DayDiffStd',	'C_DayDiff2Mean', 'C_DayDiff2Std', 'C_DayDiff3Mean', 'C_DayDiff3Std']
    X_train = data[cols_to_include].values
...
mods.to_csv(wd + "predictions_KB_NEW.csv", index=False)
...
def build_model(data, test, exclude_cols):
    ...
    X_train = data[[col for col in data.columns if col not in dependent_variables and col not in exclude_cols]].values
    ...
    'C': [1, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0.05, 0.01]
    }
...
rfm.to_csv(wd + "predictions_KB_1.csv", index=False)
tfidf_all.to_csv(wd + "predictions_KB_2.csv", index=False)
tfidf_target.to_csv(wd + "predictions_KB_3.csv", index=False)
rfm_tfidf.to_csv(wd + "predictions_KB_4.csv", index=False)

test_preds = pd.DataFrame(rfm['customer_id']).reset_index(drop=True)
for col in dependent_variables:
    test_preds[col] = 0.25*rfm[col] + 0.25*tfidf_all[col] + 0.25*tfidf_target[col] + 0.25*rfm_tfidf[col]
    
test_preds = test_preds[sub.columns]
test_preds = test_preds.sort_values(by="customer_id")    
test_preds.to_csv(wd + "GOD_IS_ULTIMATE_2.csv", index=False)
